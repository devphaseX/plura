...
const PipelineLane: React.FC<PipelaneLaneProps> = ({
  ...
  const { execute: deleteLane, status: deleteLaneStatus } = useAction(
    deleteLaneAction,
    {
      onSuccess: () => {
        toast({ title: 'Success', description: 'Lane deleted successfully' });
        router.refresh();
      },
      onError: () => {
        toast({
  ...
  const handleEditLane = () => {
    setOpen(
      <CustomModal title="Edit Lane Details" subheading="">
        <LaneForm pipelineId={pipelineId} editLaneDetails={laneDetails} />
      </CustomModal>
    );
  };

  const handleDeleteLane = () => deleteLane({ id: laneDetails.id });
  ...
  return (
    <Draggable
      ...
      index={index}
      key={laneDetails.id}
      isDragDisabled={deleteLaneStatus === 'executing'}
    >
      {(provided, snapshot) => {
              ...
              <DropdownMenu>
                <div className="bg-slate-200/30 dark:bg-background/20  h-[700px] w-[300px] px-4 relative rounded-lg overflow-visible flex-shrink-0 ">
                  ...
                  <Droppable
                    ...
                    key={laneDetails.id}
                    type="ticket"
                    isDropDisabled={deleteLaneStatus === 'executing'}
                  >
                    {(provided) => (
