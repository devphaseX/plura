...
import { deleteTicketAction } from '@/actions/ticket/handler';
...
type Props = {
  ...
  index: number;
  pipelineTickets: TicketWithTags;
  subaccountId: string;
...
export const PipelineTicket = ({
  ...
  const { execute: deleteTicket, status: deleteTicketStatus } = useAction(
    deleteTicketAction,
    {}
  );

  const editNewTicket = (ticket: TicketWithTags[number]) => {
  ...
  const handleDeleteTicket = () =>
    deleteTicket({ id: ticket.id, laneId: ticket.laneId });
  ...
  return (
    <Draggable
      ...
      index={index}
      isDragDisabled={deleteTicketStatus === 'executing'}
    >
      {(provided, snapshot) => {
        ...
        return (
          ...
          >
            <AlertDialog>
              <DropdownMenu>
                <Card
                  className="my-4 dark:bg-slate-900 bg-white 
                ...
                >
                  <CardHeader className="p-3">
                    ...
                    <div className="flex items-center flex-wrap gap-2">
                      {ticket.tagTickets.map(({ tag }) => (
                        <TagComponent
                          key={tag.id}
                          title={tag.name}
                          colourName={tag.color}
                        />
                      ))}
                    </div>
                    ...
                    <HoverCard>
                      <HoverCardTrigger>
                        <div
                          className="p-2 text-muted-foreground flex gap-2
                        hover:bg-muted transition-all rounded-lg cursor-pointer items-center"
                        >
                          <LinkIcon />
                        ...
                        <div className="flex justify-between space-x-4">
                          <Avatar>
                            ...
                            <AvatarFallback className="bg-primary">
                              {ticket.customers?.name.slice(0, 2).toUpperCase()}
                            </AvatarFallback>
                          </Avatar>
